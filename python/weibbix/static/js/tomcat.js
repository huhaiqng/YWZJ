...
//	loadServersConfigTable()
    ...
    document.getElementById("selectAllServers").onclick = function (){
    	selectAllServers();
    }
})
...
//    			var ret=data.status;   		
//    			document.getElementById("depResult").innerHTML=ret;
    		},
    	});
    })
}
...
function getDepServer(){
	...
	var i=$("#selectAllServers").children()[0];
...
function selectServerCheck(team) {
    ...
    if ($(span).hasClass("glyphicon-unchecked")) {
        ...
        $("#depServerTbody").find(".glyphicon-unchecked").each(function (){
        	m=m+1;
        });
        if (m==0){
